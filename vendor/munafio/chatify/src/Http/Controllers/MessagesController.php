...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            ...
            if($fetch){
                $userAvatar = asset('/uploads/' . config('chatify.user_avatar.folder') . '/' . $fetch->avatar);
            }
        }
    ...
    public function fetch(Request $request)
    {
        ...
        if ($totalMessages < 1) {
            $response['messages'] ='<p class="message-hint center-el"><span>Rašykite \'Labas\' ir pradėkite susirašinėti</span></p>';
    ...
    public function getContacts(Request $request)
    {
        ...
        if (count($usersList) > 0) {
            ...
            $contacts = '<p class="message-hint center-el"><span>Jūsų kontaktų sąrašas tusčias</span></p>';
        }
    ...
    public function search(Request $request)
    {
        ...
        if($records->total() < 1){
            $getRecords = '<p class="message-hint center-el"><span>Nėra ka parodyti.</span></p>';
        }
        // send the response
        return Response::json([
            'records' => $getRecords,
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('Chatify::layouts.listItem', [
                'get' => 'sharedPhoto',
                'image' => asset('storage/app/app/public/attachments/' . $shared[$i]),
            ])->render();
        }
        // send the response
        return Response::json([
            'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="message-hint"><span>Nieks nepasidalinta</span></p>',
        ], 200);
    }
