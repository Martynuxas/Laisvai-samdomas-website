...
@if($get == 'saved')
    <table class="messenger-list-item m-li-divider" data-contact="{{ Auth::user()->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ Auth::user()->id }}" data-type="user">Išsaugoti pranešimai <span>Tu</span></p>
                <span>Išsisaugok pranešimus privačiai</span>
            </td>
        </tr>
    </table>
@endif
...
<table class="messenger-list-item" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        style="background-image: url('{{ asset('/uploads/'.config('chatify.user_avatar.folder').'/'.$user->avatar) }}');">
        </div>
        </td>
        {{-- center side --}}
        <td>
