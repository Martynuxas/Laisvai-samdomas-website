    ...
    @foreach ($skelbimai as $skelbimas )
      <div class="card-h">
        <div class="card-thumbnail">
          <img src="{{ URL::to('/images')}}/{{App\Http\Controllers\SkelbimaiController::gautiPirmaNuotrauka($skelbimas->id)}}" onclick="location.href='paslauga/{{$skelbimas->id}}';" style="cursor: pointer;">
            ...
            {{$skelbimas->pavadinimas}}
              ...
              <br>
                @if(App\Http\Controllers\IsimintiController::arPaslaugaIsiminta(Auth::user()->id,$skelbimas->id) == false)
                  <i class="far fa-heart" onclick="location.href='isimintiPaslauga/{{$skelbimas->id}}';" style="cursor: pointer;">&nbsp; įsiminti</i>
                @else
                  <i class="fas fa-heart" onclick="location.href='pamirstiPaslauga/{{$skelbimas->id}}';" style="cursor: pointer;">&nbsp; pamiršti</i>
                @endif
        </div>
      </div>
    @endforeach
